...
data class TunnelConf(
    ...
    fun getDisplayName(): String {
        return when {
            flagEmoji != null -> "$flagEmoji $tunName"
            else -> tunName
        }
    }
    ...
    fun copyWithCallback(
        ...
        flagEmoji: String? = this.flagEmoji,
        serverLocation: String? = this.serverLocation,
    ): TunnelConf {
        return TunnelConf(
            id,
            tunName,
    ...
    companion object {
        ...
        // Enhanced factory method for country-based tunnels
        fun createCountryTunnel(
            config: org.amnezia.awg.config.Config,
            countryCode: String,
            ...
            val amQuick = config.toAwgQuickString(true)
            val wgQuick = config.toWgQuickString()
            val tunnelName = name ?: "${flagEmoji ?: ""} ${countryCode.uppercase()}".trim()
